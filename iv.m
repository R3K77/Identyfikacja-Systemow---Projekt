...
% Metoda LS dla wstępnych estymat parametrów
...
for n = max(na, nb+nk):N_train
    if n > max(na, nb+nk)
        x(n) = [-x(n-1:-1:n-na)', input_train(n-nk:-1:n-nk-nb+1)'] * theta_LS;
    end
end

% Wypełnianie macierzy zmiennych instrumentalnych Z
Z = zeros(N_train - max(na, nb+nk) + 1, na + nb);
for n = max(na, nb+nk):N_train
    Z_row = [-x(n-1:-1:n-na)', input_train(n-nk:-1:n-nk-nb+1)'];
    Z(n - max(na, nb+nk) + 1, :) = Z_row;
end

% Estymacja parametrów metodą IV
theta_IV = (Z' * phi) \ (Z' * y_train);
...
for i = 1:na + nb
    subplot(na + nb, 1, i);
    plot(theta_IV(i) * ones(length(y_pred), 1));
